...
function getDynamicGuide(key: string, cat: Categoria | null): GuideContent | null {
  ...
  const MAIN_CAT_GUIDES: Record<string, GuideContent> = {
    "Estructural": { title: "Especialidad: Estructural", description: "Comprende los proyectos y cálculos de la estructura portante de edificaciones, muros de contención, puentes y otras obras civiles. Incluye el diseño de sistemas resistentes a cargas gravitacionales y sísmicas conforme a normativa boliviana.", notes: ["Abarca: Proyectos Estructurales y Certificados de Estabilidad Estructural."], mainCatKey: "Estructural" },
    "Sanitario": { title: "Especialidad: Sanitario", description: "Abarca los diseños de redes de agua potable, alcantarillado sanitario y pluvial, y sistemas de tratamiento. Incluye proyectos hidro-sanitarios para edificaciones y urbanizaciones, así como certificados de validación de sistemas.", notes: ["Abarca: Proyectos Sanitarios y Certificados de Validación Sanitaria."], mainCatKey: "Sanitario" },
    "Geológico - Geotécnico": { title: "Especialidad: Geológico - Geotécnico", description: "Estudios del comportamiento del suelo y subsuelo para determinar la capacidad portante, clasificación de suelos y riesgos geológicos. Son requisito previo para cualquier proyecto de edificación o urbanización en la SIB.", notes: ["Abarca: Estudios Geotécnicos y Geológicos para edificaciones proyectadas, existentes, urbanizaciones y otros."], mainCatKey: "Geológico - Geotécnico" },
    "Eléctrico": { title: "Especialidad: Eléctrico", description: "Proyectos de redes e instalaciones eléctricas en baja, media y alta tensión, iluminación, redes domésticas e industriales. También incluye peritajes eléctricos para verificar instalaciones existentes.", notes: ["Abarca: Proyectos de Instalación Eléctrica y Peritajes Eléctricos."], mainCatKey: "Eléctrico" },
    "Mecánico": { title: "Especialidad: Mecánico", description: "Proyectos de instalaciones electromecánicas, diseño y cálculo de sistemas de ascensores, montacargas, bombeo y climatización. Cubre tanto el diseño de nuevas instalaciones como la revisión de equipos existentes.", notes: ["Abarca: Proyectos Mecánicos de diseño e instalación."], mainCatKey: "Mecánico" },
    "Ingeniería en General": { title: "Ingeniería en General", description: "Documentos técnicos transversales a varias ramas de la ingeniería. Incluye planes de contingencia ante emergencias y riesgos, así como informes técnicos y periciales requeridos por entidades públicas o privadas.", notes: ["Abarca: Planes de Contingencia e Informes Técnicos / Periciales."], mainCatKey: "Ingeniería en General" },
  };
...
//PSA (solo corrige el acento) 
  ...
  const FIELD_GUIDES: Record<string, GuideContent> = {
    ...
    "altMuro": { title: "Altura Máxima Útil de Muro (m)", description: "Altura libre medida desde la base de cimentación hasta la coronación del muro.", example: "3.50" },
    "luzPuente": { title: "Luz del Puente (m)", description: "Distancia libre entre apoyos del puente, medida en metros.", example: "25.00" },
    "superfConstruir": { title: "Superficie a Construir (m²)", description: "Área total de construcción considerando todos los niveles.", example: "1250.00" },
    "superfConstruida": { title: "Superficie Construida (m²)", description: "Área total ya construida de la edificación existente.", example: "980.00" },
    "superfTablero": { title: "Superficie del Tablero (m²)", description: "Área del tablero del puente resultante de la luz por el ancho de calzada.", example: "125.00" },
    ...
    "areaMuroCon": { title: "Área de Muro a Construir (m²)", description: "Área total de la cara expuesta del muro de contención nuevo.", example: "85.00" },
    "areaMuroRef": { title: "Área de Muro a Reforzar (m²)", description: "Área total de la cara expuesta del muro existente que será reforzado.", example: "60.00" },
    "areaMuroHA": { title: "Área de Muro Hormigón Armado (m²)", description: "Área del muro construido con hormigón armado (con acero de refuerzo).", example: "45.00" },
    "areaMuroHC": { title: "Área de Muro Hormigón Ciclópeo (m²)", description: "Área del muro construido con hormigón ciclópeo (con piedra grande embebida, sin acero).", example: "40.00" },
    "norma": { title: "Norma de Diseño", description: "Normativa técnica bajo la cual fue elaborado el proyecto.", example: "NB 1225001 (Sismorresistente), ACI 318, AASHTO LRFD" },
    "normaVerif": { title: "Norma de Verificación", description: "Normativa técnica usada para verificar o certificar el cumplimiento del proyecto.", example: "NB 1225001, CIRSOC 101" },
    "normaAplicacion": { title: "Norma Técnica de Aplicación", description: "Normativa técnica de aplicación específica para el estudio o informe elaborado.", example: "NB 1225002 (Geotécnica), NBE FL-90" },
    "potenciaInst": { title: "Potencia Instalada (kW)", description: "Suma de todas las potencias de los equipos eléctricos instalados en el sistema.", example: "45.5" },
    "potenciaDem": { title: "Potencia Demandada (kVA)", description: "Potencia máxima que demandará el sistema en condiciones normales de operación.", example: "38.2" },
    "tensionAlim": { title: "Tensión de Alimentación (V)", description: "Tensión nominal del sistema de alimentación eléctrica.", example: "220 / 380" },
    "superfTerreno": { title: "Superficie del Terreno (m²)", description: "Área total del terreno donde se emplaza o emplazará el proyecto.", example: "450.00" },
    "volMovTierras": { title: "Volumen de Movimiento de Tierras (m³)", description: "Volumen total de tierra a excavar, rellenar o mover durante la ejecución del proyecto.", example: "320.50" },
    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      ...
      {info && (
        <div style={{ background: theme.boxBg, border: `1px solid ${theme.border}`, borderRadius: 12, padding: "16px", fontSize: 12 }}>
          ...
          <button onClick={() => { if (info) onSelect(info); }} style={{ marginTop: 12, width: "100%", background: theme.accent, color: isDark ? "#0a1a12" : "#fff", border: "none", borderRadius: 8, padding: "10px 0", fontWeight: 800, fontSize: 13, cursor: "pointer" }}>
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div
        ...
        style={{ border: `2px dashed ${dragOver ? theme.accent : theme.border}`, borderRadius: 16, background: dragOver ? (isDark ? "rgba(34,197,94,0.10)" : "rgba(16,185,129,0.07)") : (isDark ? "rgba(255,255,255,0.03)" : "rgba(0,0,0,0.02)"), padding: "36px 20px", textAlign: "center", cursor: "pointer", transition: "all 0.2s ease" }}
      >
      ...
      <button onClick={() => alert("Integración con Google Drive próximamente")} style={{ width: "100%", background: "transparent", border: `1px solid ${theme.border}`, borderRadius: 10, padding: "11px 16px", fontWeight: 700, fontSize: "0.82em", color: theme.textSec, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}>
        <svg width="16" height="16" viewBox="0 0 87.3 78" fill="none"><path d="M6.6 66.85l3.85 6.65c.8 1.4 1.95 2.5 3.3 3.3L27.5 53H0c0 1.55.4 3.1 1.2 4.5L6.6 66.85z" fill="#0066da"/><path d="M43.65 25L29.9 1.2C28.55 2 27.4 3.1 26.6 4.5L1.2 48.5A9 9 0 000 53h27.5L43.65 25z" fill="#00ac47"/><path d="M73.55 76.8c1.35-.8 2.5-1.9 3.3-3.3l1.6-2.75 7.65-13.25c.8-1.4 1.2-2.95 1.2-4.5H60.1L73.55 76.8z" fill="#ea4335"/><path d="M43.65 25L57.4 1.2A9.06 9.06 0 0053.3.05H34C31.5.05 29.2 1.3 27.9 3.35L43.65 25z" fill="#00832d"/><path d="M60.1 53H27.5L13.75 76.8c1.35.8 2.9 1.2 4.5 1.2h50.8c1.6 0 3.15-.45 4.5-1.2L60.1 53z" fill="#2684fc"/><path d="M73.4 26.5L60.7 4.5c-.8-1.4-1.95-2.5-3.3-3.3L43.65 25l16.45 28h27.45c0-1.55-.4-3.1-1.2-4.5L73.4 26.5z" fill="#ffba00"/></svg>
        Subir desde Google Drive
      </button>
      {files.length > 0 && (
        <div style={{ background: theme.cardBg, border: `1px solid ${theme.border}`, borderRadius: 12, overflow: "hidden" }}>
          <div style={{ padding: "10px 14px", borderBottom: `1px solid ${theme.border}`, fontSize: "0.78em", color: theme.accent, fontWeight: 800, textTransform: "uppercase", letterSpacing: 1 }}>📁 {files.length} archivo{files.length > 1 ? "s" : ""}</div>
      ...
      {guide.notes && guide.notes.length > 0 && (
        <div style={{ background: isDark ? "rgba(245,158,11,0.1)" : "#fffbeb", border: `1px solid ${isDark ? "rgba(245,158,11,0.3)" : "#fcd34d"}`, borderRadius: 14, padding: "16px" }}>
          ...
          {guide.notes.map((note, i) => <p key={i} style={{ fontSize: 12, color: isDark ? "#fcd34d" : "#92400e", margin: "0 0 6px 0", lineHeight: 1.6, fontWeight: 500 }}>• {note}</p>)}
        </div>
      )}
    </div>
  );
...
const mainCatKey: string | null = isUploadKey ? null : (() => {
  ...
  return (
    <div style={{
      ...
      // ── CLAVE: el contenedor ocupa toda la altura del sticky y el scroll
      //    ocurre solo dentro del área de contenido (.gs), no en la página ──
      overflow: "hidden",
    }}>
      <style>{`
        ...
        /* ── Contenido: hace scroll internamente ── */
        .fg-body {
          flex: 1;
          overflow-y: auto;
          padding: 24px;
...
<div className="fg-header">
  <div style={{ fontSize: 11, color: theme.accent, textTransform: "uppercase", letterSpacing: 2, marginBottom: 6, fontWeight: 800 }}>
    {isUploadKey ? "📎 Carga de Documentos" : isMapField ? "💡 Asistente S.I.B." : "💡 Asistente S.I.B."}
  </div>
              ...
              <>
                ...
                {guide?.notes && guide.notes.length > 0 && (
                  <div style={{ background: isDark ? "rgba(245,158,11,0.1)" : "#fffbeb", border: `1px solid ${isDark ? "rgba(245,158,11,0.3)" : "#fcd34d"}`, borderRadius: 14, padding: "20px" }}>
                    ...
                    {guide.notes.map((note, i) => <p key={i} style={{ fontSize: 13, color: isDark ? "#fcd34d" : "#92400e", margin: "0 0 8px 0", lineHeight: 1.6, fontWeight: 500 }}>• {note}</p>)}
                  </div>
                )}
              </>
            )}
